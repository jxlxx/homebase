...
- Cloudflare account with the `jxlxx.org` zone delegated to Cloudflare nameservers, an **API token** that has Zone → DNS:Edit + Zone:Read permissions (export as `CLOUDFLARE_API_TOKEN`), and the Zone ID handy (export as `CLOUDFLARE_ZONE_ID`)
...
2. **DNS** – provisions the `jxlxx.org` zone on Cloudflare and creates A records for every service (including Authelia at `auth.jxlxx.org`):
   ...
   This stack now targets Cloudflare. Make sure the domain’s nameservers point to Cloudflare and that both `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ZONE_ID` are exported before running Terragrunt. The stack reads the Hetzner node IP from the `apps` dependency and creates `git`, `matrix`, `hedgedoc`, `excalidraw`, `home`, and `auth` A records.
